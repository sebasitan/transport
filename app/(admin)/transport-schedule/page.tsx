...
export default function TransportSchedulePage() {
  ...
  const stats = useMemo(() => {
    const activeVehiclesCount = new Set(slots.filter(s => s.vehicle_id != null).map(s => typeof s.vehicle_id === 'string' ? s.vehicle_id : s.vehicle_id._id)).size
    return {
      total: slots.length,
  ...
  const vehicleSlotsMap = useMemo(() => {
    ...
    slots.forEach(slot => {
      const vId = slot.vehicle_id == null ? null : typeof slot.vehicle_id === 'object' ? slot.vehicle_id._id : slot.vehicle_id
      if (!vId) return
